...
public class TFIDFItemScorer extends AbstractItemScorer {
    ...
    private SparseVector makeUserVector(long user) {
        ...
        double averageUserRating = total / count;
...
//                long id = p.getItemId();
//                profile.add(model.getItemVector(id));
                //Weighted User Profile
                long id = p.getItemId();
                MutableSparseVector temp = model.getItemVector(id).mutableCopy();
                double ecofficience = p.getValue() - averageUserRating;
                temp.multiply(ecofficience);
                profile.add(temp);
            }
        }
